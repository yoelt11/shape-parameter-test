#!/usr/bin/env python3
"""
Derivative and Hessian Evaluation: Testing parameter reduction approaches
...
def create_ground_truth_functions_with_derivatives():
    ...
    def sinusoidal_dxx(X, Y):
        """∂²f/∂x² for sinusoidal."""
        return -4 * jnp.pi**2 * jnp.sin(2 * jnp.pi * X) * jnp.cos(2 * jnp.pi * Y)
    ...
    def sinusoidal_dxy(X, Y):
        ...
        return -4 * jnp.pi**2 * jnp.cos(2 * jnp.pi * X) * jnp.sin(2 * jnp.pi * Y)
    ...
    def gaussian_mixture_dxx(X, Y):
        ...
        for (cx, cy), scale in zip(centers, scales):
            dx = (X - cx) / scale**2
            result += ((dx**2 - 1/scale**2)) * jnp.exp(-((X - cx)**2 + (Y - cy)**2) / (2 * scale**2))
        return result
    
    def gaussian_mixture_dyy(X, Y):
        """∂²f/∂y² for gaussian mixture."""
        centers = [(-0.5, -0.5), (0.5, 0.5), (0.0, 0.0)]
        scales = [0.3, 0.2, 0.4]
        result = jnp.zeros_like(X)
        for (cx, cy), scale in zip(centers, scales):
            dy = (Y - cy) / scale**2
            result += ((dy**2 - 1/scale**2)) * jnp.exp(-((X - cx)**2 + (Y - cy)**2) / (2 * scale**2))
        return result
    
    def gaussian_mixture_dxy(X, Y):
        """∂²f/∂x∂y for gaussian mixture."""
        centers = [(-0.5, -0.5), (0.5, 0.5), (0.0, 0.0)]
        scales = [0.3, 0.2, 0.4]
        result = jnp.zeros_like(X)
        for (cx, cy), scale in zip(centers, scales):
            dx = (X - cx) / scale**2
            dy = (Y - cy) / scale**2
            result += dx * dy * jnp.exp(-((X - cx)**2 + (Y - cy)**2) / (2 * scale**2))
        return result
    ...
    def anisotropic_dx(X, Y):
        """∂f/∂x for anisotropic."""
        return 3 * jnp.pi * jnp.cos(3 * jnp.pi * X) * jnp.cos(jnp.pi * Y) + 2.5 * jnp.pi * Y * jnp.cos(5 * jnp.pi * X * Y)
    
    def anisotropic_dy(X, Y):
        """∂f/∂y for anisotropic."""
        return -jnp.pi * jnp.sin(3 * jnp.pi * X) * jnp.sin(jnp.pi * Y) + 2.5 * jnp.pi * X * jnp.cos(5 * jnp.pi * X * Y)
    
    def anisotropic_dxx(X, Y):
        """∂²f/∂x² for anisotropic."""
        return -9 * jnp.pi**2 * jnp.sin(3 * jnp.pi * X) * jnp.cos(jnp.pi * Y) - 12.5 * jnp.pi**2 * Y**2 * jnp.sin(5 * jnp.pi * X * Y)
    
    def anisotropic_dyy(X, Y):
        """∂²f/∂y² for anisotropic."""
        return -jnp.pi**2 * jnp.sin(3 * jnp.pi * X) * jnp.cos(jnp.pi * Y) - 12.5 * jnp.pi**2 * X**2 * jnp.sin(5 * jnp.pi * X * Y)
    
    def anisotropic_dxy(X, Y):
        """∂²f/∂x∂y for anisotropic."""
        return -3 * jnp.pi**2 * jnp.cos(3 * jnp.pi * X) * jnp.sin(jnp.pi * Y) + 2.5 * jnp.pi * jnp.cos(5 * jnp.pi * X * Y) - 12.5 * jnp.pi**2 * X * Y * jnp.sin(5 * jnp.pi * X * Y)
    ...
    return {
        ...
        'gaussian_mixture': {
            ...
            'dxx': gaussian_mixture_dxx,
            'dyy': gaussian_mixture_dyy,
            'dxy': gaussian_mixture_dxy,
...
def create_models_with_derivatives():
    """Create models that can compute derivatives and Hessians."""
    ...
    def standard_model_evaluate_with_derivatives(X, params):
        """Evaluate standard model with derivatives and Hessians."""
        mus = params[:, 0:2]
        ...
        hess_phi = jnp.zeros((X.shape[0], params.shape[0], 2, 2))
        
        # Compute quadratic terms for Hessian
        quad_terms = jnp.einsum('nki,kij,nkj->nk', diff, inv_covs, diff)
        
        # ∂²φ/∂x² = φ * ((∂φ/∂x)² - inv_cov_11)
        hess_phi = hess_phi.at[:, :, 0, 0].set(
            phi * (quad_terms[:, :, 0]**2 - inv_covs[:, 0, 0])
        )
        
        # ∂²φ/∂y² = φ * ((∂φ/∂y)² - inv_cov_22)
        hess_phi = hess_phi.at[:, :, 1, 1].set(
            phi * (quad_terms[:, :, 1]**2 - inv_covs[:, 1, 1])
        )
        
        # ∂²φ/∂x∂y = φ * ((∂φ/∂x)(∂φ/∂y) - inv_cov_12)
        hess_phi = hess_phi.at[:, :, 0, 1].set(
            phi * (quad_terms[:, :, 0] * quad_terms[:, :, 1] - inv_covs[:, 0, 1])
        )
        hess_phi = hess_phi.at[:, :, 1, 0].set(hess_phi[:, :, 0, 1])
        
        # Hessian components
        d2f_dx2 = jnp.dot(hess_phi[:, :, 0, 0], weights)
        d2f_dy2 = jnp.dot(hess_phi[:, :, 1, 1], weights)
        d2f_dxy = jnp.dot(hess_phi[:, :, 0, 1], weights)
    ...
    def advanced_shape_transform_evaluate_with_derivatives(X, params):
        """Evaluate Advanced Shape Transform with derivatives and Hessians."""
        mus = params[:, 0:2]
        epsilons = params[:, 2]
        scales = params[:, 3]
        weights = params[:, 4]
...
def run_derivative_hessian_test(ground_truth_name, ground_truth_funcs, models, n_seeds=5):
    """Run test for derivatives and Hessians across multiple seeds."""
    
    # Create test data
    x = jnp.linspace(-1, 1, 20)  # Smaller grid for derivative computation
    ...
    for seed in seeds:
        for model_name, model in models.items():
            ...
            def create_loss_fn(evaluate_fn):
                def loss_fn(params):
                    f, df_dx, df_dy, d2f_dx2, d2f_dy2, d2f_dxy = evaluate_fn(X_eval, params)
            ...
            derivative_error = (jnp.mean((df_dx - target_dx) ** 2) + 
                              jnp.mean((df_dy - target_dy) ** 2) + 
                              jnp.mean((d2f_dx2 - target_dxx) ** 2) + 
                              jnp.mean((d2f_dy2 - target_dyy) ** 2) + 
                              jnp.mean((d2f_dxy - target_dxy) ** 2)) / 5.0
...
def run_derivative_hessian_evaluation():
    """Run comprehensive evaluation of derivatives and Hessians."""
    
    print("="*80)
    print("DERIVATIVE AND HESSIAN EVALUATION")
    print("="*80)
    ...
    for gt_name, gt_info in ground_truths.items():
        ...
        results = run_derivative_hessian_test(gt_name, gt_info, models, n_seeds=5)
        all_results[gt_name] = results
    
    return all_results, ground_truths

def create_derivative_visualization(all_results, ground_truths):
    ...
    fig, axes = plt.subplots(3, len(ground_truth_names), figsize=(20, 12))
    fig.suptitle('Derivative and Hessian Evaluation: Parameter Reduction Approaches', fontsize=16, fontweight='bold')
    ...
    # Second row: Final loss comparison
    for col, gt_name in enumerate(ground_truth_names):
        ax = axes[1, col]
    ...
    plt.tight_layout()
    plt.savefig('derivative_hessian_evaluation.png', dpi=300, bbox_inches='tight')
    plt.show()
    
    return fig

def analyze_derivative_results(all_results, ground_truths):
    """Analyze results of derivative evaluation."""
    
    print("\n" + "="*80)
    print("DERIVATIVE AND HESSIAN ANALYSIS")
    print("="*80)
    ...
    print(f"5. **Hessian Computation**: Full second derivative computation included")

def main():
    """Main function to run derivative and Hessian evaluation."""
    ...
    print("4. Function-specific performance across different ground truths")
    
    # Run evaluation
    all_results, ground_truths = run_derivative_hessian_evaluation()
    
    # Create comprehensive visualization
    create_derivative_visualization(all_results, ground_truths)
    
    # Analyze results
    analyze_derivative_results(all_results, ground_truths)
    
    print("\n" + "="*80)
    print("DERIVATIVE AND HESSIAN EVALUATION COMPLETE")
    print("="*80)
    print("The evaluation demonstrates:")
    print("1. **Derivative accuracy** maintained across approaches")
    print("2. **Hessian computation** performance comparison")
    print("3. **Training efficiency** with derivative constraints")
    print("4. **Function-specific** performance patterns")
