#!/usr/bin/env python3
"""
Standard with Shape Transform comparison: Testing standard approach with shape parameter transform.
This script compares standard approach with and without shape parameter transform.
"""
...
def create_comparison_models():
    """Create models for comparison: Standard vs Standard with Shape Transform."""
    ...
    # Standard approach with shape transform
    def standard_with_shape_transform_initialize(n_kernels=25, key=None):
        if key is None:
            key = jax.random.PRNGKey(42)
        
        # Standard parameterization with shape transform: [mu_x, mu_y, epsilon, weight]
    ...
    def standard_with_shape_transform_evaluate(X, params):
        """Evaluate standard model with shape transform applied."""
        mus = params[:, 0:2]
        epsilons = params[:, 2]
        weights = params[:, 3]
        
        # Apply shape transform to generate standard parameters
        ...
        # Convert to log_sigmas and angles for standard evaluation
        log_sigma_x = jnp.log(sx)
        log_sigma_y = jnp.log(sy)
        
        # Create the full parameter array for standard evaluation
        full_params = jnp.zeros((params.shape[0], 6))
        full_params = full_params.at[:, 0:2].set(mus)  # means
        full_params = full_params.at[:, 2].set(log_sigma_x)  # log_sigma_x
        full_params.at[:, 3].set(log_sigma_y)  # log_sigma_y
        full_params = full_params.at[:, 4].set(theta)  # angles
        full_params = full_params.at[:, 5].set(weights)  # weights
        
        # Use standard evaluation
        return standard_model_evaluate(X, full_params)
    ...
    return {
        ...
        'standard_with_shape_transform': {
            'initialize': standard_with_shape_transform_initialize,
            'evaluate': standard_with_shape_transform_evaluate,
            'name': 'Standard + Shape Transform',
            'color': 'blue'
        }
    }
...
def run_multi_ground_truth_evaluation():
    ...
    print("="*80)
    print("STANDARD WITH SHAPE TRANSFORM EVALUATION")
    print("="*80)
...
def create_comprehensive_visualization(all_results, ground_truths):
    ...
    model_names = ['standard', 'standard_with_shape_transform']
    colors = ['red', 'blue']
    labels = ['Standard (Complex)', 'Standard + Shape Transform']
    
    fig, axes = plt.subplots(4, len(ground_truth_names), figsize=(20, 16))
    fig.suptitle('Standard vs Standard with Shape Transform: Multi-Ground Truth Evaluation', fontsize=16, fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig('standard_with_shape_transform_evaluation.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def analyze_multi_ground_truth_results(all_results, ground_truths):
    ...
    print("\n" + "="*80)
    print("STANDARD WITH SHAPE TRANSFORM ANALYSIS")
    print("="*80)
    ...
    for gt_name in ground_truth_names:
        ...
        shape_transform_grad_time = np.mean(all_results[gt_name]['standard_with_shape_transform']['grad_times'])
        speedup = (standard_grad_time - shape_transform_grad_time) / standard_grad_time * 100
        
        standard_final_loss = np.mean(all_results[gt_name]['standard']['final_losses'])
        shape_transform_final_loss = np.mean(all_results[gt_name]['standard_with_shape_transform']['final_losses'])
        quality_improvement = (standard_final_loss - shape_transform_final_loss) / standard_final_loss * 100
    ...
    print(f"3. **Shape Transform Impact**: How shape transform affects standard approach")
    print(f"4. **Parameter Efficiency**: Reduced parameters with maintained expressibility")

def main():
    """Main function to run standard with shape transform evaluation."""
    
    print("Standard with Shape Transform Evaluation")
    print("="*80)
    print("This script evaluates standard approach with and without shape parameter transform.")
    ...
    print("\n" + "="*80)
    print("STANDARD WITH SHAPE TRANSFORM EVALUATION COMPLETE")
    print("="*80)
    print("The evaluation shows how shape transform affects the standard approach:")
    print("1. **Parameter reduction** with shape transform")
    print("2. **Speed improvements** through systematic control")
    print("3. **Quality trade-offs** with reduced parameters")
    print("4. **Expressibility maintenance** with shape transform")
