#!/usr/bin/env python3
"""
Efficient Derivative Evaluation: Testing parameter reduction approaches
...
def create_efficient_models_with_derivatives():
    """Create models that can compute derivatives efficiently."""
    ...
    def standard_model_evaluate_with_derivatives(X, params):
        """Evaluate standard model with derivatives (efficient version)."""
        mus = params[:, 0:2]
        ...
        # First derivatives (efficient vectorized version)
        # ∂φ/∂x = -φ * inv_cov * (x - μ)
    ...
    def advanced_shape_transform_evaluate_with_derivatives(X, params):
        """Evaluate Advanced Shape Transform with derivatives (efficient version)."""
        mus = params[:, 0:2]
        epsilons = params[:, 2]
        scales = params[:, 3]
        weights = params[:, 4]
...
def run_efficient_derivative_test(ground_truth_name, ground_truth_funcs, models, n_seeds=3):
    """Run test for derivatives across multiple seeds (efficient version)."""
    
    # Create test data
    x = jnp.linspace(-1, 1, 12)  # Even smaller grid for efficiency
    ...
    for seed in seeds:
        for model_name, model in models.items():
            ...
            params = model['initialize'](n_kernels=16, key=key)  # Fewer kernels for efficiency
            ...
            for epoch in range(200):  # Shorter training for efficiency
...
def run_efficient_derivative_evaluation():
    """Run comprehensive evaluation of derivatives (efficient version)."""
    
    print("="*80)
    print("EFFICIENT DERIVATIVE EVALUATION")
    print("="*80)
    
    # Get ground truth functions and models
    ground_truths = create_ground_truth_functions_with_derivatives()
    models = create_efficient_models_with_derivatives()
    
    all_results = {}
    
    for gt_name, gt_info in ground_truths.items():
        print(f"\nTesting {gt_info['name']} function with derivatives...")
        results = run_efficient_derivative_test(gt_name, gt_info, models, n_seeds=3)
        all_results[gt_name] = results
    
    return all_results, ground_truths

def create_efficient_derivative_visualization(all_results, ground_truths):
    ...
    fig.suptitle('Efficient Derivative Evaluation: Parameter Reduction Approaches', fontsize=16, fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig('efficient_derivative_evaluation.png', dpi=300, bbox_inches='tight')
    plt.show()
    
    return fig

def analyze_efficient_derivative_results(all_results, ground_truths):
    """Analyze results of derivative evaluation."""
    
    print("\n" + "="*80)
    print("EFFICIENT DERIVATIVE ANALYSIS")
    print("="*80)
    ...
    print(f"5. **Efficient Computation**: Vectorized operations avoid compilation issues")

def main():
    """Main function to run efficient derivative evaluation."""
    ...
    print("4. Efficient vectorized computation to avoid JAX compilation issues")
    
    # Run evaluation
    all_results, ground_truths = run_efficient_derivative_evaluation()
    
    # Create comprehensive visualization
    create_efficient_derivative_visualization(all_results, ground_truths)
    
    # Analyze results
    analyze_efficient_derivative_results(all_results, ground_truths)
    
    print("\n" + "="*80)
    print("EFFICIENT DERIVATIVE EVALUATION COMPLETE")
    print("="*80)
    ...
    print("4. **Efficient computation** avoiding JAX compilation issues")
